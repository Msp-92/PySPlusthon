...
def compile_proto_files(directory: Path = Path("proto"), main_package: bool = True):
    for child in directory.iterdir():
        ...
        protoc.main([
            "grpc_tools.protoc",
            "--proto_path=proto",
            "--python_out=../PySplusthon/proto",
            "--pyi_out=../PySplusthon/proto",
            "--grpc_python_out=../PySplusthon/proto",
            f"proto/{path}"
        ])


def fix_compiled_files(directory: Path = Path("../PySplusthon/proto"), main_package: bool = True):
...
def generate_parameters_for_requests():
    ...
    init_file = Path("..", "PySplusthon", "proto", "requests", "__init__.py")
